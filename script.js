document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getOrCreateError(field) {
      const id = 'error-' + (field.id || field.name || Math.random().toString(36).slice(2));
      ...
      if (field.hasAttribute('required') && !VALIDATORS.required(value)) {
        ...
        if ((type === 'email' || field.dataset.validate === 'email') && !VALIDATORS.email(value)) {
          ...
          if (!VALIDATORS.minlength(value, min)) {
            errorMessage = getErrorMessage('minlength', min);
          }
        }
      }
    ...
    contactForm.addEventListener('blur', function (e) {
      ...
      if (
        field.matches('input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="reset"]), textarea, select')
      ) {
        validateField(field);
      }
    }, true);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "סרטי-קופיקו משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '04bc544a-6868-4c1a-aa5e-32230dc72720',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '04bc544a-6868-4c1a-aa5e-32230dc72720';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '04bc544a-6868-4c1a-aa5e-32230dc72720';
